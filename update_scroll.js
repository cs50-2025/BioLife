...
content = content.replace(
  ...
  'const handleTouchMove = (e: React.TouchEvent) => { if (!touchStartY.current) return; const currentY = e.touches[0].clientY; const diff = currentY - touchStartY.current; const target = e.currentTarget; if (diff > 50 && !isCollapsed && target.scrollTop <= 0) { setIsCollapsed(true); touchStartY.current = null; } else if (diff < -50 && isCollapsed) { setIsCollapsed(false); touchStartY.current = null; } };'
);

  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-stone-800">Good morning, Alex! 🌿</h1>
          <p className="text-stone-500 mt-1">Your plants are looking great today.</p>
        </div>
      ...
      <div className="bg-emerald-600 rounded-3xl p-6 text-white shadow-lg shadow-emerald-600/20 relative overflow-hidden">
        <div className="relative z-10 w-2/3">
          <h2 className="text-xl font-bold mb-2">Check Plant Health</h2>
          <p className="text-emerald-100 text-sm mb-4">Scan your plant to get instant AI diagnosis and care tips.</p>
            ...
            {todaysTasks.map(task => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-stone-500 truncate">{task.amount} • {task.time}</p>
                </div>
                <button 
      ...
      <div>
        <div className="flex justify-between items-end mb-4">
          <h2 className="text-xl font-bold text-stone-800">My Plants</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {plants.slice(0, 5).map(plant => (
            <Link key={plant.id} to={`/plants/${plant.id}`} className="bg-white rounded-2xl p-3 border border-stone-100 shadow-sm hover:shadow-md transition-shadow group">
              ...
              <p className="text-xs text-stone-500 truncate">{plant.type}</p>
          ...
          <Link to="/plants" className="bg-stone-100 rounded-2xl p-3 border border-stone-200 border-dashed flex flex-col items-center justify-center text-stone-500 hover:bg-stone-200 hover:text-stone-700 transition-colors min-h-[200px]">
            ...
            <span className="font-medium text-sm">Add Plant</span>
          </Link>
        </div>
      </div>
    </div>

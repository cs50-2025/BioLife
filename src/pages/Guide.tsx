...
export default function Guide() {
  ...
  const topics = [
    { title: 'Watering 101', desc: 'Learn the soak and dry method', icon: Droplets, color: 'text-blue-500 bg-blue-50' },
    { title: 'Lighting Basics', desc: 'Direct vs indirect light explained', icon: Sun, color: 'text-amber-500 bg-amber-50' },
    { title: 'Pest Control', desc: 'Identify and treat common bugs', icon: Bug, color: 'text-red-500 bg-red-50' },
    { title: 'Soil & Repotting', desc: 'Choosing the right mix', icon: Leaf, color: 'text-emerald-500 bg-emerald-50' },
  ];
  ...
  const generateLesson = async (topic: string) => {
    ...
    try {
      ...
      setActiveLesson({ title: topic, content: response.text || 'Failed to generate lesson.' });
    } catch (error) {
      console.error(error);
      setActiveLesson({ title: topic, content: 'An error occurred while generating the lesson. Please try again.' });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-3xl p-6 md:p-10 border border-stone-100 shadow-sm min-h-[50vh]">
          <div className="flex items-center gap-3 mb-8 pb-6 border-b border-stone-100">
            ...
            <div>
              ...
              <p className="text-emerald-600 font-medium text-sm">AI-Generated Lesson</p>
            </div>
          </div>
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12 text-stone-400 space-y-4">
              ...
              <p className="font-medium animate-pulse">Generating your personalized lesson...</p>
            </div>
          ) : (
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-stone-800">AI Care Lessons</h1>
        <p className="text-stone-500 mt-1">Generate personalized, interactive lessons on any plant care topic.</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-3xl p-6 md:p-8 text-white shadow-lg shadow-emerald-600/20 relative overflow-hidden">
        <div className="relative z-10 max-w-xl">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-bold text-emerald-100 tracking-wider uppercase text-sm">Custom Lesson</span>
          </div>
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input 
                ...
                placeholder="e.g., How to propagate a Monstera..." 
      ...
      {/* Popular Topics */}
      <div>
        <h2 className="text-xl font-bold text-stone-800 mb-4">Popular Topics</h2>
